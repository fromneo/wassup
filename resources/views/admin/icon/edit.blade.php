        ...
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @endif
                    <form method="POST" action="{{ route('icons.update', ['id' => $icon->id]) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="animation" type="text" class="form-control{{ $errors->has('animation') ? ' is-invalid' : '' }}" name="animation" value="{{ $icon->animation }}" required autofocus>
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="category_id" class="form-control{{ $errors->has('category_id') ? ' is-invalid' : '' }}" name="category_id">
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{$category->id}}" @if($category->id == $icon->category_id) selected="selected" @endif>{{$category->name}}</option>
                                        @foreach($category->childrenList as $child)
                                            <option value="{{$child->id}}" @if($child->id == $icon->category_id) selected="selected" @endif>--{{$child->name}}</option>
                                        @endforeach
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="filter_id" class="form-control{{ $errors->has('filter_id') ? ' is-invalid' : '' }}" name="filter_id">
                                    ...
                                    @foreach($filters as $filter)
                                        <option value="{{$filter->id}}" @if($filter->id == $icon->filter_id) selected="selected" @endif>{{$filter->name}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="plan_id" class="form-control{{ $errors->has('plan_id') ? ' is-invalid' : '' }}" name="plan_id">
                                    ...
                                    @foreach($plans as $plan)
                                        <option value="{{$plan->id}}" @if($plan->id == $icon->plan_id) selected="selected" @endif>{{$plan->name}}</option>
                                    @endforeach
                                </select>
